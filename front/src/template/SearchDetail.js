...
function getMenu(detailTogFun, setDetailMenu){
    ...
    axios({
        method: "get",
        url:
            `http://192.168.240.250:8080/api/v1/franchisee/` + detailTogFun.clickMarkerBN + `/menus`
    }).then(function (res) {
...
function SearchDetail({ options, detailTogFun }) {
    ...
    if(detailTogFun.detailTog === true && doMenuFun === false){
        ...
        axios({
            method: "get",
            url:
                `http://192.168.240.250:8080/api/v1/franchisee/` + detailTogFun.clickMarkerBN
        }).then(function (res) {
    ...
    return (
        <>
            <Offcanvas className={'searchDeatil-offcanvas'} show={detailTogFun.detailTog} onHide={searchResultClose} placement={'start'} {...options}>
                ...
                <Offcanvas.Body className={'searchDeatil-offcanvasbody--bodyzone'}>
                    <div className="offcanvas-body--titlezone">
                        <div>
                            <img src={"http://192.168.240.250:8080"+detailInfo.firstImg} alt="디폴트 이미지" width='300px'/>
                        </div>
                        <div>
                    ...
                    <div className="searchDeatil-offcanvasbody--buttonzone">
                        <Tabs
                            ...
                            onSelect={(option) => {if(option=='menu'){
                            ...
                            <Tab eventKey="info" title="정보">
                                <ListGroup variant="flush">
                                    <ListGroup.Item>주소 이미지 /{detailInfo.address.road}</ListGroup.Item>
                                    <ListGroup.Item>전화번호 이미지 / {detailInfo.tel}</ListGroup.Item>
                            ...
                            <Tab eventKey="menu" title="메뉴">
                                {detailMenu.map((ele, idx) => {
                                    return (
                                        <Card className={'searchDeatil-offcanvasbody--cardzone mb-3'}>
                                            <Card.Body>
                                                <Card.Img style={{width:'140px', float: 'left', marginRight: '10px', borderRadius:'6px'}} variant="left" src={"http://192.168.240.250:8080"+detailMenu[idx].image.path} alt="대체이미지"/>
                                                <Card.Title>{detailMenu[idx].name}</Card.Title>
                                                <Card.Subtitle className="mb-2 priceText">{detailMenu[idx].price} 원</Card.Subtitle>
                                                <Card.Text>{detailMenu[idx].description}</Card.Text>
                                            </Card.Body>
                                        </Card>
                                    );
                                })}
                            ...
                            <Tab eventKey="review" title="리뷰">
                                <Card className={'searchDeatil-offcanvasbody--cardzone mb-2'}>
                                    <Card.Body>
